...
import { logMainframe, logError } from '../../utils/logger';
...
export class ZOSMFClient {
  ...
  constructor(config?: Partial<ZOSMFConfig>) {
    ...
    logMainframe('ZOSMFClient initialized', 'SYSTEM', {
   ...
   */
  async authenticate(tsoId: string, password: string): Promise<AuthenticationResult> {
    try {
      logMainframe('Authentication attempt', tsoId, { endpoint: '/zosmf/services/authenticate' });
      ...
      const ltpaToken = response.headers['set-cookie']?.find(
        (cookie: string) => cookie.startsWith('LtpaToken2')
      );

      if (!ltpaToken) {
        logError('Authentication failed: No LTPA token received', undefined, { tsoId });
        return {
          success: false,
      ...
      logMainframe('Authentication successful', tsoId);
  ...
  async logout(token: string): Promise<{ success: boolean; message: string }> {
    try {
      logMainframe('Logout attempt', 'SYSTEM', { endpoint: '/zosmf/services/logout' });

      // Call z/OSMF logout endpoint to invalidate session
      await this.axiosInstance.post(
        '/zosmf/services/logout',
        {},
        {
          headers: {
            'Cookie': token,
      ...
      logMainframe('Logout successful - mainframe session closed', 'SYSTEM');
      ...
      logError('Mainframe logout failed', error);
   ...
   */
  async submitJob(jclContent: string, token: string): Promise<JobSubmitResult> {
    try {
      logMainframe('Submitting JCL job', 'SYSTEM', { jclLength: jclContent.length });
      ...
      logMainframe('JCL job submitted', 'SYSTEM', {
      ...
      logError('JCL submission failed', error);
   ...
   */
  async getJobStatus(jobName: string, jobId: string, token: string): Promise<JobStatus> {
    try {
      ...
      logError('Failed to get job status', error, { jobName, jobId });
   ...
   */
  async getJobOutput(jobName: string, jobId: string, token: string): Promise<string> {
    try {
      ...
      for (const file of files) {
        const contentResponse = await this.axiosInstance.get(
          `/zosmf/restjobs/jobs/${jobName}/${jobId}/files/${file.id}/records`,
          {
            headers: {
              'Cookie': token,
            },
      ...
      logError('Failed to get job output', error, { jobName, jobId });
   ...
   */
  async listDatasets(pattern: string, token: string): Promise<DatasetInfo[]> {
    try {
      const response = await this.axiosInstance.get(
        `/zosmf/restfiles/ds`,
        {
      ...
      logError('Failed to list datasets', error, { pattern });
   ...
   */
  async readDataset(datasetName: string, token: string): Promise<string> {
    try {
      const response = await this.axiosInstance.get(
        `/zosmf/restfiles/ds/${datasetName}`,
        {
          headers: {
            'Cookie': token,
          },
      ...
      logError('Failed to read dataset', error, { datasetName });
   ...
   */
  async getSpoolFiles(jobName: string, jobId: string, token: string): Promise<any[]> {
    try {
      logMainframe('Getting spool files', 'SPOOL', { jobName, jobId });
      ...
      logMainframe('Spool files retrieved', 'SPOOL', { 
        jobName, 
        jobId, 
      ...
      logError('Failed to get spool files', error, { jobName, jobId });
  ...
  async getSpoolContent(
    ...
    try {
      logMainframe('Getting spool content', 'SPOOL', { jobName, jobId, fileId });

      const response = await this.axiosInstance.get(
        `/zosmf/restjobs/jobs/${jobName}/${jobId}/files/${fileId}/records`,
        {
          headers: {
            'Cookie': token,
          },
      ...
      logMainframe('Spool content retrieved', 'SPOOL', { 
        jobName, 
        jobId, 
        fileId,
      ...
      logError('Failed to get spool content', error, { jobName, jobId, fileId });
   ...
   */
  async getDatasetMember(datasetName: string, token: string): Promise<string> {
    try {
      logMainframe('Getting dataset member', 'DATASET', { datasetName });
      ...
      const response = await this.axiosInstance.get(
        `/zosmf/restfiles/ds/${encodedDataset}`,
        {
          headers: {
            'Cookie': token,
          },
      ...
      logMainframe('Dataset member retrieved', 'DATASET', { 
        datasetName,
        length: content.length 
      });
      ...
      logError('Failed to get dataset member', error, { datasetName });
   ...
   */
  async listDatasetMembers(datasetName: string, token: string): Promise<string[]> {
    try {
      logMainframe('Listing dataset members', 'DATASET', { datasetName });

      const response = await this.axiosInstance.get(
        `/zosmf/restfiles/ds/${datasetName}/member`,
        {
          headers: {
            'Cookie': token,
          },
      ...
      const memberNames = members.map((m: any) => m.member);
      
      logMainframe('Dataset members listed', 'DATASET', { 
        datasetName,
      ...
      logError('Failed to list dataset members', error, { datasetName });
   ...
   */
  private handleAuthError(error: any, tsoId: string): AuthenticationResult {
    if (axios.isAxiosError(error)) {
      const axiosError = error as AxiosError;

      if (axiosError.code === 'ETIMEDOUT' || axiosError.code === 'ECONNABORTED') {
        logError('Authentication timeout', error, { tsoId });
        return {
          success: false,
      ...
      if (axiosError.response?.status === 401) {
        logError('Invalid credentials', error, { tsoId });
        return {
          success: false,
      ...
      if (axiosError.response?.status === 403) {
        logError('Access forbidden', error, { tsoId });
        return {
          success: false,
      ...
      if (!axiosError.response) {
        logError('Network error', error, { tsoId });
        return {
          success: false,
    ...
    logError('Authentication failed', error, { tsoId });
    return {
      success: false,
   ...
   */
  private createError(error: any, message: string): Error {
    if (axios.isAxiosError(error)) {
      const axiosError = error as AxiosError;
...
export const zosmfClient = new ZOSMFClient();
