...
export class OpenAIService {
  ...
  constructor() {
    ...
    if (!apiKey) {
      logWarn('OpenAI API key not configured. AI features will be disabled.');
    ...
    logInfo(`OpenAI service initialized with model: ${this.model}`);
  }
   ...
   */
  public async analyzeJiraIssue(issueData: any): Promise<{
    summary: string;
    intent: string;
    claimDetails: any;
    keyPoints: string[];
...
Extract the intent and claim details from this issue.`;
    ...
    try {
      ...
      logError('OpenAI analysis failed', error);
   ...
   */
  public async generateXMLFromJira(issueData: any): Promise<string> {
    if (!this.isConfigured) {
      throw new Error('OpenAI API key not configured');
    }
...
Generate a clean, well-structured XML document.`;
    ...
    try {
      ...
      logError('XML generation failed', error);
      throw this.handleOpenAIError(error);
    }
  }
   ...
   */
  public async generateTaskBreakdown(requirements: string): Promise<{
    tasks: Array<{
      id: string;
      title: string;
      description: string;
...
Create a logical task breakdown with workflow sequence.`;
    ...
    try {
      ...
      logError('Task breakdown generation failed', error);
      throw this.handleOpenAIError(error);
    }
  }
...
NEEDS_CONFIRMATION: true`;
   ...
   */
  public async explainOutput(content: string, context?: string): Promise<{
    summary: string;
...
7. Respond ONLY with valid JSON, no additional text`;
    ...
    try {
      ...
      logError('Output explanation failed', error);
      throw this.handleOpenAIError(error);
    }
  }
