...
class AIAnalysisService {
     ...
     */
    async analyzeCSRContent(issueData, progressCallback) {
        ...
        try {
            ...
            (0, logger_1.logInfo)('Starting AI analysis for CSR content', {
...
Respond ONLY with valid JSON in this exact structure:
{
  ...
  "confidence": 85
            ...
            (0, logger_1.logInfo)('AI analysis completed successfully', {
                issueKey: issueData.key,
            ...
            (0, logger_1.logError)('AI analysis failed', error, { issueKey: issueData.key });
            throw new Error(`Failed to analyze CSR content: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * Build comprehensive analysis prompt from Jira issue data
     */
    buildAnalysisPrompt(issueData) {
     ...
     */
    emitProgress(callback, step, message) {
        if (callback) {
            callback({
                step,
                message,
     ...
     */
    async reanalyzeWithCorrections(issueData, corrections, progressCallback) {
        if (!this.openAIService.isAvailable()) {
...
Re-analyze with these corrections in mind.`;
        try {
            ...
            (0, logger_1.logError)('Re-analysis failed', error);
